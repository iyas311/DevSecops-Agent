...
# ─── Page Config ─────────────────────────────────────────────────────────────
st.set_page_config(
    page_title="CISA – Cloud Identity Security Assistant",
    page_icon="🛡️",
    layout="wide",
...
# ─── Tool metadata ────────────────────────────────────────────────────────────
TOOL_META = {
    "list_iam_users":         ("👥", "Listing IAM users"),
    "get_iam_user":           ("🔍", "Fetching user details"),
    "list_iam_roles":         ("🎭", "Listing IAM roles"),
    "get_iam_role":           ("🔍", "Fetching role trust policy"),
    "list_user_mfa_devices":  ("🔐", "Checking MFA devices"),
    "analyze_iam_policy":     ("📋", "Analysing IAM policy"),
    "run_iam_security_audit": ("🛡️", "Running comprehensive security audit"),
    "list_policies":          ("📄", "Listing policies"),
    "get_policy":             ("📋", "Fetching policy document"),
}
...
def build_report(messages):
    lines = [f"# CISA Security Report\n_Generated {datetime.now().strftime('%Y-%m-%d %H:%M')}_\n\n---\n"]
    for m in messages:
        role = "**You**" if m["role"] == "user" else "**CISA**"
        lines.append(f"### {role}\n{m['content']}\n")
        if m.get("tools"):
...
with st.sidebar:
    ...
    <div class="sidebar-logo">
      ...
      <div class="sidebar-logo-text">
        <h2>CISA</h2>
        <p>Cloud Identity Security Assistant</p>
      </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    if st.button("🔍 Run IAM Audit", use_container_width=True, key="btn_audit"):
        st.session_state.trigger_msg = (
            "Run a comprehensive IAM security audit covering root account, password policy, "
            "MFA, access key rotation, inactive users, inline policies, wildcard policies, "
            "and unused roles."
        )
    if st.button("👥 List IAM Users", use_container_width=True, key="btn_users"):
        st.session_state.trigger_msg = "List all IAM users in this account with their creation dates and attached policies."
    if st.button("🎭 List IAM Roles", use_container_width=True, key="btn_roles"):
        st.session_state.trigger_msg = "List all IAM roles in this account."
...
<div class="main-header">
  <div class="main-title">🔒 CISA</div>
  <div class="main-subtitle">AI-powered AWS Cloud Identity Security Assistant</div>
</div>
""", unsafe_allow_html=True)
...
for msg in st.session_state.messages:
    if msg["role"] == "user":
        ...
        st.markdown(
            ...
            f'<span class="msg-label-inline cisa-label">CISA</span></div>',
            unsafe_allow_html=True
        )
...
# ─── Chat input ───────────────────────────────────────────────────────────────
prompt = st.chat_input("Ask about your AWS IAM security…")
...
if prompt:
    ...
    with st.status("🤖 CISA is analysing…", expanded=True) as status_box:
        feed = st.empty()
        completed_html = ""
